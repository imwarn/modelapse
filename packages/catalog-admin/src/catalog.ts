...
import {
  DIRECT_SMOKE_CASE_SLUG,
  DIRECT_SMOKE_FAMILY,
  DIRECT_SMOKE_PACK,
  DIRECT_SMOKE_PROMPT,
  DIRECT_SMOKE_VARIANT,
  DIRECT_SMOKE_VERSION,
  directSmokeDefinitionSha256,
} from "./direct-smoke.js";
...
export interface DeepSeekSmokeBootstrapResult {
  readonly providerSourceId: string;
  readonly definitionSourceId: string;
  readonly providerId: string;
  readonly endpointId: string;
...
async function ensureNamedProvider(
  client: PoolClient,
  input: {
  ...
  await client.query(
    `INSERT INTO modelapse.providers
     ...
     VALUES ($1, $2, $3)
     ON CONFLICT (slug) DO NOTHING`,
    [input.slug, input.name, input.homepage],
  );
  ...
  const provider = result.rows[0];
  if (!provider) {
    throw new Error("Provider could not be resolved: " + input.slug);
  }
  if (provider.name !== input.name || provider.homepage !== input.homepage) {
    throw new Error(
      "Provider slug " + input.slug + " conflicts with existing catalog data",
    );
  }
  return provider.id;
}

async function ensureNamedDirectEndpoint(
  client: PoolClient,
  input: {
    readonly providerId: string;
  ...
  const existing = await client.query<{
    ...
    `SELECT id, hostname, source_id
      ...
      WHERE provider_id = $1
        AND path = 'first_party_direct'
        AND base_url = $2`,
    [input.providerId, input.baseUrl],
  );

  if (existing.rows[0]) {
    if (existing.rows[0].hostname !== input.hostname) {
      throw new Error(
        "Existing direct endpoint has an incompatible hostname: " +
          input.hostname,
      );
    }
    if (!existing.rows[0].source_id) {
      await client.query(
        `UPDATE modelapse.provider_endpoints
            SET source_id = $2
          WHERE id = $1`,
        [existing.rows[0].id, input.sourceId],
      );
    }
    return existing.rows[0].id;
  }
  ...
  const inserted = await client.query<{ id: string }>(
    `INSERT INTO modelapse.provider_endpoints
      (provider_id, path, base_url, hostname, valid_from, source_id)
     VALUES ($1, 'first_party_direct', $2, $3, now(), $4)
     RETURNING id`,
    [
      input.providerId,
      input.baseUrl,
  ...
  const id = inserted.rows[0]?.id;
  if (!id) throw new Error("Direct provider endpoint insert failed");
  return id;
}

async function ensureDirectSmokeFamily(
  client: PoolClient,
  definitionSourceId: string,
): Promise<string> {
  await client.query(
    `INSERT INTO modelapse.test_families
     ...
     ON CONFLICT (slug) DO NOTHING`,
    [
      DIRECT_SMOKE_FAMILY.slug,
      DIRECT_SMOKE_FAMILY.name,
      DIRECT_SMOKE_FAMILY.origin,
      definitionSourceId,
      JSON.stringify({ purpose: "first-party-direct-production-smoke" }),
    ],
  );
  ...
  const family = result.rows[0];
  if (!family) throw new Error("Direct Smoke Test Family could not be resolved");
  if (
    family.name !== DIRECT_SMOKE_FAMILY.name ||
    family.origin !== DIRECT_SMOKE_FAMILY.origin
  ) {
    throw new Error(
      "Direct Smoke Test Family slug conflicts with existing catalog data",
    );
  }
  if (!family.canonical_source_id) {
    await client.query(
...
async function ensureDirectSmokeVariant(
  client: PoolClient,
  familyId: string,
): Promise<string> {
  await client.query(
    ...
    [
      familyId,
      DIRECT_SMOKE_VARIANT.slug,
      DIRECT_SMOKE_VARIANT.name,
      DIRECT_SMOKE_VARIANT.category,
      DIRECT_SMOKE_VARIANT.artifactType,
    ],
  );
  ...
  const result = await client.query<{
    ...
    `SELECT id, name, category, artifact_type
      ...
      WHERE family_id = $1
        AND slug = $2`,
    [familyId, DIRECT_SMOKE_VARIANT.slug],
  );
  const variant = result.rows[0];
  if (!variant) throw new Error("Direct Smoke Test Variant could not be resolved");
  if (
    variant.name !== DIRECT_SMOKE_VARIANT.name ||
    variant.category !== DIRECT_SMOKE_VARIANT.category ||
    variant.artifact_type !== DIRECT_SMOKE_VARIANT.artifactType
  ) {
    throw new Error(
      "Direct Smoke Test Variant conflicts with existing catalog data",
    );
  }
  return variant.id;
}

async function ensureDirectSmokeVersionAndCase(
  client: PoolClient,
  variantId: string,
  prompt: BlobDescriptor,
  definitionSourceId: string,
  ...
  let version = (
    await client.query<{
      ...
      `SELECT id, status, definition_sha256, source_id
        ...
        WHERE variant_id = $1
          AND version = $2`,
      [variantId, DIRECT_SMOKE_VERSION],
    )
  ).rows[0];
  ...
  if (!version) throw new Error("Direct Smoke Test Version could not be resolved");
  if (version.definition_sha256 !== definitionSha256) {
    throw new Error(
      "Direct Smoke Test Version exists with a different immutable definition",
    );
  }
  if (version.status === "retired") {
  ...
  let testCase = (
    await client.query<{
      ...
      `SELECT id, case_type, visibility, status, prompt_blob_sha256
        ...
        WHERE test_version_id = $1
          AND slug = $2`,
      [version.id, DIRECT_SMOKE_CASE_SLUG],
    )
  ).rows[0];
  ...
  if (!testCase) {
    if (version.status !== "draft") {
      throw new Error(
        "Published Direct Smoke Test Version is missing its immutable Test Case",
      );
    }
    const inserted = await client.query<{
      id: string;
  ...
  if (
    ...
    throw new Error(
      "Direct Smoke Test Case conflicts with existing catalog data",
    );
  }
...
export class PgCatalogAdmin {
  ...
  async bootstrapDeepSeekSmoke(input: {
    readonly runnerBuild: string;
  }): Promise<DeepSeekSmokeBootstrapResult> {
    if (!input.runnerBuild.trim()) {
      throw new Error("runnerBuild is required");
    }

    const prompt = await this.blobStore.put({
      bytes: DIRECT_SMOKE_PROMPT,
      mimeType: "text/plain; charset=utf-8",
      visibility: "public",
    });
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        "SELECT pg_advisory_xact_lock(hashtext('modelapse:catalog:deepseek-smoke:v1'))",
      );
      ...
      const definitionSourceId = await ensureSource(client, {
        sourceType: "modelapse_definition",
        url: "https://github.com/imwarn/modelapse/blob/main/packages/catalog-admin/src/direct-smoke.ts",
        title: "Modelapse provider-neutral first-party direct smoke TestPack",
      });

      const providerId = await ensureNamedProvider(client, {
        slug: "deepseek",
        name: "DeepSeek",
      ...
      const testFamilyId = await ensureDirectSmokeFamily(
        client,
        definitionSourceId,
      );
      const testVariantId = await ensureDirectSmokeVariant(
        client,
        testFamilyId,
      );
      const { versionId, caseId } = await ensureDirectSmokeVersionAndCase(
        client,
        testVariantId,
        prompt,
        definitionSourceId,
      ...
      return {
        ...
        testCaseId: caseId,
        promptSha256: prompt.sha256,
        definitionSha256: directSmokeDefinitionSha256(),
      };
    } catch (error) {
      await client.query("ROLLBACK");
      throw error;
...
export const directSmokeDefinition = DIRECT_SMOKE_PACK;
